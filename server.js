...
const API_TOKEN =
  process.env.API_TOKEN || 'Dsl8UTgQ3fJFR7zrjXOAKhtxBZ2cmWVSM9ECuP5Ne1kap6qH';
...
app.post('/render', async (req, res) => {
  try {
    ...
    if (!video_url || !audio_url)
      return res
        .status(400)
        .json({ error: 'video_url and audio_url required' });
    ...
    await sh('ffmpeg', [
      '-y',
      // 🔁 loop infinito del video (se cortará con -shortest, a la duración del audio)
      '-stream_loop',
      '-1',
      '-i',
      inV,
      ...
      [
        'hflip', // espejo
        'scale=iw*1.05:ih*1.05', // zoom leve
        'crop=iw:ih', // mantiene lienzo
        'rotate=3*PI/180', // 3 grados
        'eq=contrast=1.08', // +~8% contraste
      ].join(','),
      '-map',
      '0:v:0',
      '-map',
      '1:a:0',
      '-shortest', // ⬅️ el encode termina cuando termina el audio
      '-c:v',
      'libx264',
      '-preset',
      'veryfast',
